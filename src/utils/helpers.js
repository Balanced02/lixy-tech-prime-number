...
export const generatePrimeNumbersToVal = (noOfPrimes, lastArr) => {
  var arr = [2];
  ...
  for (var i = arr.length === 1 ? 3 : (arr[arr.length - 1] + 2); arr.length < noOfPrimes; i += 2) {
...
export const checkIfPrime = (num, arr) => {
  if (!arr || !arr.length) {
    return false
  }
